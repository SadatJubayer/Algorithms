...
void Graph::initializeState(){
	for(int i=0; i<this->vertexCount; i++){
		this->key[i] = 0; // 0=not calculate yet, 1=yes, calculted
...
int Graph::isAllKeyTrue(){
	for(int i=0; i<this->vertexCount; i++){
		if(this->key[i]==0)
			return 0; // not shortest path yet
	}
	return 1; //  done
...
void main(){
	Graph g(5);
	g.addEdgeDirectedWeight(0, 1, 10); g.addEdgeDirectedWeight(0, 3, 5);	
	g.addEdgeDirectedWeight(1, 2, 1);  g.addEdgeDirectedWeight(1, 3, 2);
	g.addEdgeDirectedWeight(2, 4, 4); 
	g.addEdgeDirectedWeight(3, 1, 3);  g.addEdgeDirectedWeight(3, 2, 9);
	g.addEdgeDirectedWeight(3, 4, 2); 
	g.addEdgeDirectedWeight(4, 0, 7);  g.addEdgeDirectedWeight(4, 2, 6); 
